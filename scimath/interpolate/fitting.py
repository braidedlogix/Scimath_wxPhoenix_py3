...
class DataFit(HasPrivateTraits):
    ...
    def set_xy(self, x, y):
        ...
        assert len(x) == y.shape[
            -1], "x and y arrays must have the same length"
    ...
    def block_interp(self, x):
        ...
        indices = numpy.atleast_1d(
            numpy.clip(indices, 0, numpy.Inf).astype(numpy.int))
...
class Spline(DataFit):
    ...
    def set_xy(self, x, y):
        ...
        if not self.using_special_case():
            ...
            self._representation = interpolate.splrep(
                x, y, k=order, s=self.smoothness)
        self.initialized = True
