...
class FormatTest(TestCase):
    ...
    def test_unicode(self):
        assert format_expansion(
            self.volt, mul=" ", pow_func=unicode_powers
        ) == "A\u207B\u00B9 kg m\u00B2 s\u207B\u00B3"
    ...
    def test_unicode_division(self):
        assert format_expansion(
            self.volt, mul=" ", div=True,
            pow_func=unicode_powers) == "kg m\u00B2/(A s\u00B3)"

    def test_unicode_division_simple_denominator(self):
        assert format_expansion(
            self.joule, mul=" ", div=True,
            pow_func=unicode_powers) == "kg m\u00B2/s\u00B2"

    def test_unicode_division_no_denominator(self):
        assert format_expansion(
            self.coulomb, mul=" ", div=True, pow_func=unicode_powers) == "A s"

    def test_unicode_symbol(self):
        assert format_expansion(
            self.siemens, mul=" ",
            pow_func=unicode_powers) == "\u2126\u207B\u00B9"

    def test_unicode_symbol_division(self):
        assert format_expansion(
            self.siemens, mul=" ", div=True,
            pow_func=unicode_powers) == "1/\u2126"

    def test_unicode_rational_power(self):
        assert format_expansion(
            {
                'm': 0.5
            }, mul=" ", pow_func=unicode_powers) == "m^0.5"
    ...
    def test_TeX_division_no_denominator(self):
        assert format_expansion(
            self.coulomb, mul="\/", div=True, pow_func=tex_powers) == "A\/s"

    def test_TeX_division_no_numerator_simple_denominator(self):
        assert format_expansion(
            self.hertz, mul="\/", div=True, pow_func=tex_powers) == "1/s"

    def test_unicode_rational_power(self):
        assert format_expansion(
            {
                'm': 0.5
            }, mul="\/", pow_func=tex_powers) == "m^{0.5}"
