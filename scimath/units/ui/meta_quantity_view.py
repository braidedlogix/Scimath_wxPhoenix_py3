...
class MetaQuantityView(View):
    ...
    def __init__(self, *args, **traits):
        ...
        name_editor = EnumEditor(
            name="known_names", object='handler', evaluate=evaluate)

        name_item = Item(
            name='name', label='Name', editor=name_editor, id='name_item')

        super(MetaQuantityView, self).__init__(
            Item(
                name='name', editor=name_editor),
            Item(
                name='family_name', label='Measure of'),
            Item(name='units'),
            *args,
            **traits)
