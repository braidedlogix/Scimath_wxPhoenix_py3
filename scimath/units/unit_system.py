...
class UnitSystem(HasTraits):
    ...
    def __init__(self, name, families={}):
        ...
        if families:
            for fam in families:
                ...
                self.add_family(fam, families[fam][0], families[1],
                                families[2])
    ...
    def add_family(self, family_name, fam_unit, description=None,
                   inverse=None):
    ...
    def get_family_name(self, quant_name):
        ...
        try:
            ...
            logger.exception('Could not find a family name for %s' %
                             quant_name)
            fam_name = None
        return fam_name
    ...
    def get_inverse_log_name(self, quant_name):
        ...
        try:
            inv_name = self.unit_manager.unit_families[self.get_family_name(
                quant_name)].inverse
        except KeyError:
            logger.exception('Could not find an inverse log name for %s' %
                             quant_name)
            inv_name = None
        return inv_name
